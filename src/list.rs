...
pub fn get_fult(p:String) -> String {
    ...
    if read.len() > 99 {
        //update fump
        let next_fult_name = (fult_name.parse::<i32>().unwrap() + i32::from(100)).to_string();
        //println!("base fult : {} || len : {} || next fult : {}",fult_name.clone(),read.clone().len(),next_fult_name.clone());
        ...
        files::make_file(fult_address.clone());
        return fult_address;
    }
}
